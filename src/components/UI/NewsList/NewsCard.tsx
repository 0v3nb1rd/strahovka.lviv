...
const NewsCard: React.FC<NewsCardProps> = ({ category, date, maxLength, title, views, imageUrl, reviews, likes }) => {
  return (
    <li>
      <div
        // href="#"
        className={cn(
          'relative block overflow-hidden rounded-2xl bg-white shadow transition-all delay-75 duration-100 hover:shadow-xl',
          {
        ...
        {maxLength === undefined && (
          <div className="flex gap-10 px-5 py-4">
            ...
            <button className="group ml-auto flex items-center gap-2">
              <BiLike
                fontSize={22}
                className={`transition group-hover:scale-110 ${likes.liked ? 'text-red-500' : ''}`}
              />
              <span className="countdown w-5 text-base">
